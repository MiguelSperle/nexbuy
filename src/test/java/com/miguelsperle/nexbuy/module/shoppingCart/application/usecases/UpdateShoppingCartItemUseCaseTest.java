...
public class UpdateShoppingCartItemUseCaseTest {
    @InjectMocks
    private UpdateShoppingCartItemUseCaseImpl updateShoppingCartItemUseCase;
    ...
    public void should_update_shopping_cart_item() {
        ...
        final ShoppingCart updatedShoppingCart = shoppingCart.withTotalAmount(DecimalUtils.valueOf(200));

        Mockito.when(this.shoppingCartRepository.save(Mockito.any())).thenReturn(updatedShoppingCart);

        final UpdateShoppingCartItemUseCaseInput updateShoppingCartItemUseCaseInput = UpdateShoppingCartItemUseCaseInput.with(
                shoppingCart.getId(),
                shoppingCartItem.getShoppingCartId(),
                2
        );

        this.updateShoppingCartItemUseCase.execute(updateShoppingCartItemUseCaseInput);
