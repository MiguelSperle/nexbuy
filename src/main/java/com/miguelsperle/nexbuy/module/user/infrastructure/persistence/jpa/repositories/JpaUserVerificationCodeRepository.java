...
public interface JpaUserVerificationCodeRepository extends JpaRepository<JpaUserVerificationCodeEntity, String> {
    @Modifying
    @Query(nativeQuery = true, value = "DELETE FROM user_verification_codes uvc WHERE uvc.user_id = :userId")
    ...
    @Query(nativeQuery = true, value = "SELECT * FROM user_verification_codes uvc WHERE uvc.code = :code")
    Optional<JpaUserVerificationCodeEntity> findByCode(@Param("code") String code);
}
